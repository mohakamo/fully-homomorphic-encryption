...
void FHE_Cipher_Text::Refresh(FHE_Cipher_Text &c_cipher) const {
...
#ifdef _CHECK_NOISE_
  //  std::cout << "Refresh noise: " << ThNoise << " -> ";
  ...
  R_Ring_Vector s_pp;
  if (sk != NULL) {
    FHE::Bit_Decomposition((*sk)[c.second].Tensor_Product((*sk)[c.second]), (*sk)[c.second].Get_q(), s_pp);
...
#ifdef _CHECK_NOISE_
  int n  = (*pk)[c.second].Get_Noof_Columns() - 1;
  long int log_q1 = NumBits((*pk)[c.second - 1].Get_q());
  long int log_q2 = NumBits((*pk)[c.second].Get_q());
  ThNoise = ThNoise + c.first.Get_d() * (n * (n + 1)) * log_q1 * log_q2 * 512;
  if (ThNoise * 2 >= c2.Get_q()) {
    std::cout << ThNoise << std::endl;
    std::cout << "Added to noise: " << c.first.Get_d() * (n * (n + 1)) * log_q1 * log_q2 * 512 << std::endl;
    std::cout << "c2.Get_q() = " << c2.Get_q() << std::endl;
  }
...
#ifdef _CHECK_NOISE_
  //  std::cout << "Mult_Const noise: " << ThNoise << " * " << n << " -> ";
...
#ifdef _CHECK_NOISE_
  //  std::cout << "Add noise: " << c1_cipher.ThNoise << " + " << c2_cipher.ThNoise << " -> ";
  ThNoise = c1_cipher.ThNoise + c2_cipher.ThNoise;
  //  std::cout << ThNoise << std::endl;
...
FHE_Cipher_Text FHE_Cipher_Text::Mult(FHE_Cipher_Text &c1_cipher, FHE_Cipher_Text &c2_cipher) const {
  ...
  ZZ ThNoise = ZZ::zero();
#ifdef _CHECK_NOISE_
  //  std::cout << "Mult noise: " << c1_cipher.ThNoise << " * " << c2_cipher.ThNoise << " -> ";
  ThNoise = c1_cipher.ThNoise * c2_cipher.ThNoise * c1.first.Get_Field_Expansion();
  //  std::cout << ThNoise << std::endl;
