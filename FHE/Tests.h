...
private:
  ...
  bool test_FHE(GLWE_Type type) {
    ...
    FHE_Params params = fhe.Setup(2, 1, type, ZZ(INIT_VAL, modules[0]));
  ...
  bool test_FHE_Switch_Keys(void) {
    ...
    for (int modul_index = 0; modul_index < 1; modul_index++) {
      ...
      ZZ modul_zz = ZZ(INIT_VAL, modul);
      FHE_Params params = fhe.Setup(2, 2, types[j], ZZ(INIT_VAL, modul));
    for (int s = 0; s < 10; s++) {
      std::cout << s << std::endl;
      Pair<FHE_Secret_Key_Type, FHE_Public_Key_Type> sk_pk = fhe.Key_Gen(params);
      
      R_Ring_Number message = R_Ring_Number::Uniform_Rand(modul_zz, params[0].d);
      
      FHE_Cipher_Text c = fhe.Encrypt(params, &sk_pk.second, message);
      // check initial correction
      R_Ring_Number decoded_initial_message = c.Decrypt(params, sk_pk.first);
      if (decoded_initial_message != message.Get_Clamped(modul_zz)) {
	...
	R_Ring_Vector secret_tensored_prime;
	Bit_Decomposition(secret_tensored, secret_tensored.Get_q(), secret_tensored_prime);

	R_Ring_Vector powersof2_tensored_c1;
	Powersof2(tensored_c1.first, tensored_c1.first.Get_q(), powersof2_tensored_c1);
	...
	FHE_Cipher_Text tc(tensored_c1, &sk_pk.second, modul_zz, ZZ::zero());
	c.Refresh(tc);
  ...
  bool test_FHE_Two_Mult(GLWE_Type type) {
	...
	FHE_Cipher_Text c[3];
	ZZ bound = ZZ(INIT_VAL, pow(modul, 0.333));
	for (int j = 0; j < 3; j++) {
	  message[j] = R_Ring_Number::Uniform_Rand(bound, params[0].d);
	  message[j].Increase_Modul(to_ZZ(modul));
  ...
  bool test_FHE_One_Add(GLWE_Type type, FHE_Operation operation_type, int message_modul = 2, int noof_operations = 1) {
	...
	for (int i = 0; i < dimension; i++) {
      ...
      R_Ring_Vector c_powers2;
      Powersof2(c3, c3.Get_q(), c_powers2);
      assert(c3.Get_q() == sk_tensored.Get_q());
      R_Ring_Vector sk_tensored_bitdec;
      Bit_Decomposition(sk_tensored, c3.Get_q(), sk_tensored_bitdec);
